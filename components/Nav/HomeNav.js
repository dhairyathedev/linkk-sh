...
export default function HomeVav({isNewUser, onClick}) {
  ...
  return (
    ...
    >
      ...
      <ul className="flex items-center gap-2 text-sm font-medium text-gray-500">
        <li className={`${isNewUser ? "hidden" : "block"}`}>
          <Link
            href="/app"
          ...
          >
             {isAuthenticated || !loading? "Dashboard" :  "SignIn"}
          </Link>
        </li>
        <li className={`${isNewUser ? "block" : "hidden"}`}>
          <a
            className="font-secondary rounded-full border border-black bg-black py-1.5 px-5 text-sm text-white transition-all hover:bg-white hover:text-black"
            onClick={onClick}
          >
